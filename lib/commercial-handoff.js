...
function buildCommercialHandoff(state = {}) {
  ...
  const canonicalValue = (field, fallback = '') => canonical[field]?.value || fallback;
  const business = canonicalValue('business', state.businessType);
  const products = canonicalValue('products', state.productsOrServices);
  const furniture = /móveis planejados|moveis planejados/i.test(`${business} ${products}`);
  if (furniture) {
    const phoneValue = canonicalValue('visitorPhone', state.visitorPhone);
    const visitorPhone = typeof phoneValue === 'object' ? phoneValue.display : phoneValue;
    const rejected = Array.isArray(state.rejectedFacts) ? state.rejectedFacts.map((fact) => fact.value).filter(Boolean) : [];
    const message = [
      'Olá. Conversei com o assistente virtual da Propagação Digital e quero continuar com um responsável.',
      '',
      `Nome: ${clean(canonicalValue('name', state.customerName))}`,
      `Negócio: ${clean(business)}`,
      `Produtos: ${clean(products)}`,
      `Clientes: ${clean(canonicalValue('customerTypes', state.customerTypes))}`,
      `Canais: ${clean(canonicalValue('channels', state.salesChannels))}`,
      `Objetivo: ${clean(canonicalValue('goal', state.goals))}`,
      `Pedido visual: ${Array.isArray(state.visualRequests) && state.visualRequests.length ? `solicitou ${state.visualRequests.join(' e ')}` : 'não informado'}`,
      `Galeria: ${state.visualActionStatus === 'RENDERED' || state.generalVisualAccepted ? 'aceitou visualizar modelo geral' : 'não aceitou modelo geral'}`,
      `Telefone do visitante: ${visitorPhone || 'não informado'}`,
      `Preferência de contato: ${state.contactPreference === 'whatsapp' ? 'WhatsApp' : clean(state.contactPreference, 'não informada')}`,
      ...(rejected.length ? [`Fato rejeitado: ${rejected.join(', ')} — informação incorreta, não usar`] : []),
      'Próximo passo: responsável avaliar um site básico compatível com o negócio e continuar pelo WhatsApp.'
    ].join('\n');
    return { number: OFFICIAL_WHATSAPP, message, url: `https://wa.me/${OFFICIAL_WHATSAPP}?text=${encodeURIComponent(message)}` };
  }
  const visualRequests = Array.isArray(state.visualRequests) && state.visualRequests.length
    ? state.visualRequests.join(' e ')
    : 'Nenhuma registrada';
  const phone = state.visitorPhone ? clean(state.visitorPhone).replace(/\) 9 (\d{4}-\d{4})$/, ') 9$1') : 'não informado';
