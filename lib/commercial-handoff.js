...
function buildCommercialHandoff(state = {}) {
  ...
  const message = [
    ...
    `Canais atuais: ${clean(state.salesChannels)}`,
    `Objetivo: ${clean(state.goals)}`,
    `Interesses: ${/whatsapp/i.test(state.salesChannels || '') ? 'página/cardápio para pedidos pelo WhatsApp' : clean(state.preferences)}`,
    ...
    'Próximo passo: falar com responsável para avaliar material específico',
