...
const FIELDS = ['customerName', 'businessType', 'productsOrServices', 'salesChannels', 'goals', 'painPoints', 'preferences', 'visitorBudget'];
...
function buildConversationRequest({ model, messages = [], currentMessage, state = {} }) {
  ...
  return {
    ...
    instructions: [
      ...
      'visitorBudget é dinheiro que o visitante declarou poder investir; não é preço da Propagação Digital. Você pode repetir esse orçamento, sem convertê-lo em preço oficial.',
      'Não responda com linguagem de estado interno como “Já tenho seu nome”, “Isso me ajuda a direcionar”, “Registrado” ou “Atualizei os dados”.',
...
async function requestConversationTurn({ apiKey, model, messages, currentMessage, state, fetchImpl = fetch, timeoutMs = CONVERSATION_TIMEOUT_MS }) {
  ...
  try {
    ...
    const validation = validateConversationOutput(candidate, state);
