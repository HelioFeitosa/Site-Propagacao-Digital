...
const ALLOWED_ACTIONS = new Set(['whatsapp', 'gallery', 'service-page', 'open_visual']);
...
function resolveLocalTurn(message, currentState = {}) {
  ...
  if (/(qual e (o )?meu nome|como eu me chamo|voce sabe (o )?meu nome)/.test(value)) {
    const name = state.canonicalMemory?.name?.value || state.customerName;
    return { handled: true, reply: name ? `Seu nome é ${name}.` : 'Você ainda não me informou seu nome.' };
  }
  ...
  if (/(o que eu vendo|com o que eu trabalho|voce sabe o que eu faco|o que eu faco)/.test(value)) {
    ...
    const product = asksWhatIDo
      ? (state.canonicalMemory?.business?.value || state.businessType || state.productsOrServices)
      : (state.canonicalMemory?.products?.value || state.productsOrServices || state.businessType);
  ...
  if (/(o que eu quero melhorar|qual e (o )?meu objetivo)/.test(value)) {
    const goal = state.canonicalMemory?.goal?.value || state.goals;
    return { handled: true, reply: goal ? `Você quer ${goal}.` : 'Você ainda não me informou o que quer melhorar.' };
  }
  ...
  if (/(voce.*anotou.*(?:zap|whatsapp|telefone)|qual.*meu.*(?:zap|whatsapp|telefone))/.test(value)) {
    const phone = state.canonicalMemory?.visitorPhone?.value?.display || state.visitorPhone;
    return { handled: true, reply: phone ? `O telefone que você informou é ${phone}.` : 'Você ainda não informou seu telefone ou WhatsApp.' };
  }

  if (/(?:meu|minha).*(?:zap|whatsapp|telefone|celular).*(?:\d)/.test(value) && state.visitorPhone) {
    return { handled: true, reply: `Seu telefone para contato é ${state.visitorPhone}.` };
  }

  if (/(quais|qual).*(meus canais|canais que uso|canais de venda)/.test(value)) {
    const channels = state.canonicalMemory?.channels?.value || state.salesChannels;
    return { handled: true, reply: channels ? `Seus canais são ${channels}.` : 'Você ainda não informou seus canais.' };
  }
  ...
  const furnitureContext = /moveis planejados/.test(normalize(`${state.canonicalMemory?.business?.value || ''} ${state.businessType} ${state.productsOrServices}`));
  if (furnitureContext && /(mostre|mostrar|quero ver|tem).*(imagem|exemplo|modelo|site)/.test(value)) {
    if (state.visualActionStatus === 'RENDERED') {
      return { handled: true, reply: 'O modelo geral já foi apresentado nesta conversa.' };
    }
    return {
      handled: true,
      reply: 'Ainda não temos um exemplo específico de móveis planejados. Posso mostrar um modelo geral apenas para você conhecer a estrutura.',
      statePatch: {
        visualRequested: true,
        visualActionStatus: 'REQUESTED',
        visualConsentPending: true,
        visualRequests: [...new Set([...(state.visualRequests || []), 'exemplo de site'])]
      }
    };
  }

  if (furnitureContext && state.visualConsentPending && /(abra|abrir|pode mostrar|mostre|quero ver).*(galeria|modelo|exemplo)?/.test(value)) {
    return {
      handled: true,
      reply: 'Aqui está o exemplo visual disponível.',
      action: { type: 'open_visual', assetId: 'lume-modas-functional-demo', status: 'READY' },
      statePatch: {
        visualRequested: true,
        visualStatus: 'READY',
        visualAssetId: 'lume-modas-functional-demo',
        visualActionStatus: 'READY',
        visualConsentPending: false,
        generalVisualAccepted: true,
        galleryInterest: false
      }
    };
  }

  if (furnitureContext && /^(estou aguardando|aguardando|e ai|e agora)[.!?\s]*$/.test(value)) {
    if (state.visualActionStatus === 'RENDERED') return { handled: true, reply: 'O modelo geral foi apresentado acima. Você pode abrir o projeto pelo próprio card.' };
    if (state.visualActionStatus === 'FAILED') return { handled: true, reply: 'Não foi possível apresentar o modelo. Posso seguir em texto ou encaminhar seu pedido ao responsável.' };
    return { handled: true, reply: 'Posso continuar com a orientação em texto enquanto avaliamos o próximo passo.' };
  }
