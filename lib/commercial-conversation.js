...
function advanceCommercialConversation(current, userText) {
  ...
  const protectedTopic = /(\b(ola|oi|bom dia|boa tarde|boa noite)\b|meu nome|preco|valor|quanto custa|promocao|desconto|r\$|telefone|whatsapp|zap|link|url|cta|botao|imagem|visual|handoff|atendimento humano|falar com (o )?(helio|responsavel|alguem)|responsavel|admin|variavel|servidor|api|credencial|senha)/.test(normalize(userText));
  return {
    ...result,
