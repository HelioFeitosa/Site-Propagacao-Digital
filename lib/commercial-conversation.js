...
function firstName(text) {
  ...
  const raw = value
    ...
    .split(/\s+/)[0]
    .replace(/[^\p{L}'-]/gu, '');
  const ignored = new Set(['só', 'so', 'você', 'voce', 'empresa', 'quero', 'negócio', 'negocio', 'imagem', 'serviços', 'servicos']);
...
function advanceCommercialConversation(current, userText) {
  ...
  if (!state.customerName) {
    ...
    if (!name) return { state, reply: 'Como você gostaria que eu te chamasse?' };
