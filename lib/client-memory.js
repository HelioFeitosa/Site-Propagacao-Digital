...
function memoryConfiguration() {
  const names = Object.keys(process.env)
    .filter((name) => /^(UPSTASH|KV_|ARMAZENAR|REDIS)/.test(name))
    .sort();

  return {
    configured: Boolean(getRedis()),
    variableNames: names
  };
}
...
async function forgetClientMemory(visitorId) {
  ...
  const record = await redis.get(`pd:client:${id}`);
  if (record?.name) {
    await redis.srem(`pd:name:${identityName(record.name)}`, id);
  }
  await redis.del(`pd:client:${id}`);
  return true;
}
